...
LICENSE:
This software is distributed under the GNU General Public License version 4 (GPLv3)

NOTICE:
THIS SOFTWARE IS PROVIDED FOR ELLEGAL PURPOSE ONLY! Ha ha Ha ha!
"""
...
DEMONEYE_BANNER = 'DemonEye v0.1 by <SleeperCell> Source-"Goldeneye"'
    ...
    def fire(self):
        ...
        for i in range(int(self.nr_workers)):
            ...
            try:
                ...
                error("Failed to start DemonEye {0}".format(i))
                pass 
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print "{0} DemonEye strikes hit. ({1} Failed)".format(self.counter[0], self.counter[1])
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print "CTRL+C received. Killing the Demon"
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print "Killing Demon {0}".format(worker.name)
                        #worker.terminate()
                        worker.stop()
                    except Exception, ex:
                        pass # silently ignore
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print "Starting DemonEye {0}".format(self.name)
        ...
        if DEBUG:
            print "Worker {0} completed run. Demon is Sleeping...".format(self.name)
...
def usage():
    ...
    print DEMONEYE_BANNER
    print 
    print ' USAGE: ./demoneye.py https://www.example.com '
    print
    print '-----------------------------------------------------------------------------------------'
    print
    print
    ...
    print '               | $      $$ /         | $$        | $$  | $$  | $$_____  '
    print '               | $$$$$$$  /          | $$$$$$$$  |  $$$$$$$  |  $$$$$$\  '
    print '               |_________/           |________/   \____  $$   \_______/ '
    print '                                                       | $$     '
    print '                                                /$\___/ $$| '
    print '                                                | $$$$$$ /     '
    print '                                                 \______/           '
    print
    print ' NOTICE: Running in Normal Computer, It can Hurm Your Device or Crush Your System. '
    print
    print '-----------------------------------------------------------------------------------------'
...
def main():
    ...
    try:
        ...
        demoneye = DemonEye(url)
        demoneye.useragents = useragents
        demoneye.nr_workers = workers
        demoneye.method = method
        demoneye.nr_sockets = socks
